...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = HookNet(encoder_name=args.arch, encoder_weights=None, classes=4)

    if os.path.isfile(args.weights):
        logger.info(f"=> loading pretrained weights {args.weights} into encoder")
        state_dict = torch.load(args.weights, map_location="cpu")

        if args.weight_name == "rsp_cr":
            state_dict = state_dict["model"]
            context_state_dict = OrderedDict()
            target_state_dict = OrderedDict()

            for k, v in state_dict.items():
                if k.startswith("module.model.") and not k.startswith("module.fc"):
                    context_state_dict[k[len("module.model."):]] = v
            target_state_dict = context_state_dict.copy()

        elif args.weight_name == "dsmil":
            context_state_dict = state_dict["context_weight"]
            target_state_dict = state_dict["target_weight"]

        is_strict = False if args.weight_name == "dsmil" else True
        msg1 = model.context_branch.encoder.load_state_dict(context_state_dict, strict=is_strict)
        msg2 = model.target_branch.encoder.load_state_dict(target_state_dict, strict=is_strict)

        logger.info(f"=> loaded pretrained weights {args.weights} into encoder")
    else:
    ...
    criterion = smp.losses.DiceLoss(smp.losses.MULTICLASS_MODE, classes=[1, 2, 3], from_logits=True)
    # optimizer = optim.AdamW(model.parameters(), init_lr)
    # logger.info("=> use custom AdamW optimiser!")
    ...
    macro_iou_recorder = BestRecorder("max")
    micro_acc_recorder = BestRecorder("max")
    macro_acc_recorder = BestRecorder("max")
    raw_f1_recorders = {i: BestRecorder("max") for i in CLASSES}
    raw_iou_recorders = {i: BestRecorder("max") for i in CLASSES}
    raw_acc_recorders = {i: BestRecorder("max") for i in CLASSES}
    ...
    for epoch in range(args.epochs):
        ...
        if not args.multiprocessing_distributed or args.rank == 0:
            ...
            train_f1_macro = smp.metrics.f1_score(tp, fp, fn, tn, reduction="macro-imagewise")
            ...
            best_iou_macro, _ = macro_iou_recorder.update(val_iou_macro)
            ...
            if args.tensorboard:
                ...
                tb_writer.add_scalars(
                    "train/f1", {"micro": train_f1_micro, "macro": train_f1_macro}, epoch
                )


                tb_writer.add_scalars(
                    "val/f1", {"micro": val_f1_micro, "macro": val_f1_macro}, epoch
                )
                tb_writer.add_scalars(
                    "val/iou", {"micro": val_iou_micro, "macro": val_iou_macro}, epoch
                )
                tb_writer.add_scalars(
                    "val/acc", {"micro": val_acc_micro, "macro": val_acc_macro}, epoch
                )
            ...
            logger.info(
                ...
                f"MICRO IOU: {val_iou_micro:.4f}/{best_iou_micro:.4f}, MACRO IOU: {val_iou_macro:.4f}/{best_iou_macro:.4f}\n"
    ...
    if args.rank == 0:
        ...
        logger.info(
            "=======\n"
            f"MICRO F1: {micro_f1_recorder.best:.4f}, MACRO F1: {macro_f1_recorder.best:.4f}\n"
            f"MICRO IOU: {micro_iou_recorder.best:.4f}, MACRO IOU: {macro_iou_recorder.best:.4f}\n"
            f"MICRO ACC: {micro_acc_recorder.best:.4f}, MACRO ACC: {macro_acc_recorder.best:.4f}\n"
        )
        for cls_name in CLASSES:
            logger.info(
...
def train(loader, model, criterion, optimizer, epoch, scaler, args):
    ...
    for i, (images, masks) in enumerate(loader):
        ...
        with autocast(enabled=args.amp):
            ...
            loss = (1 - args.lam) * criterion(context_logits_mask, masks[0]) \
                    + args.lam * criterion(target_logits_mask, masks[1])  # TODO: fine-tune the lambda
...
def validate(loader, model, epoch, args):
    ...
    val_acc_micros = []
    val_acc_macros = []
    class_f1 = {i: [] for i in CLASSES}
    class_iou = {i: [] for i in CLASSES}
    class_acc = {i: [] for i in CLASSES}
    ...
    for i, (images, masks) in enumerate(loader):
        ...
        val_iou_macro = smp.metrics.iou_score(tp, fp, fn, tn, reduction="macro")
        ...
        val_iou_macros.append(val_iou_macro.item())

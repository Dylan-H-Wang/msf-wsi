...
def main_worker(gpu, ngpus_per_node, args):
        ...
        if os.path.isfile(args.weights):
            ...
            for k in list(state_dict.keys()):
                ...
                if (
                    ...
                    and not k.startswith("module.encoder.projection_head")
                    and not k.startswith("module.encoder.prototypes")
                    and not k.startswith("module.projection_head")
                    and not k.startswith("module.prototypes")
                ):
                    # remove prefix
            ...
            # logger.info(f"=> encoder is frozen!")
    ...
    # TODO - add centre_crop to avoid edge effect
    train_transform = [
    ...
    logger.info(f"=> train aug pipeline: {train_aug}")

    train_dataset = BcssSegDataset(
        args.train_data,
        train_aug,
        mask_on=True,
        frac=args.frac,  # TODO: change augs
    )
        ...
        if not args.multiprocessing_distributed or (
            ...
            if args.wandb:
                wandb.log(
                    {
                        "train/loss": loss,
                        "train/micro_f1": train_f1_micro,
                        "train/macro_f1": train_f1_macro,
                        "val/loss": loss_,
                        "val/micro_f1": val_f1_micro,
                        "val/macro_f1": val_f1_macro,
                    }
                )
                wandb.run.summary["loss"] = best_loss
                wandb.run.summary["micro_f1"] = best_f1_micro
                wandb.run.summary["macro_F1"] = best_f1_macro
            ...
            logger.info(
                ...
                f"OTHER F1: {other_f1:.4f}/{other_f1_:.4f}/{best_other_f1:.4f}, MICRO ACC: {best_acc_micro:.4f}\n"
                "======="
            )
...
def train(loader, model, criterion, optimizer, epoch, scaler, args):
    ...
    for i, (images, masks) in enumerate(loader):
        ...
        scaler.update()
        del loss, images, logits_mask, masks, pred_mask
...
def dice_score(im1, im2):
    ...
    im1 = torch.flatten(im1, start_dim=1)
    im2 = torch.flatten(im2, start_dim=1)

    # Compute Dice coefficient
    inter_sum = 2.0 * torch.sum(im1 * im2, -1)
    im_sum = torch.sum(im1, -1) + torch.sum(im2, -1)

    inter_sum[im_sum == 0] = 1.0
    im_sum[im_sum == 0] = 1.0
    scores = inter_sum / im_sum

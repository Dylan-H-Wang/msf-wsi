...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_dataset = PaipSegDataset(args.train_data, train_aug, frac=args.frac, fold=args.fold)
    ...
    val_dataset = PaipSegDatasetVal(args.val_data, val_aug, fold=args.fold)
...
def validate(loader, model, epoch, args):
    ...
    with torch.no_grad():
        ...
        for i, (images, target_masks) in enumerate(loader):

            # measure data loading time
            data_time.update(time.time() - end)

            target_imgs_split = torch.split(images[0], 128)

            preds = []
            for img1 in target_imgs_split:
                if args.gpu is not None:
                    img1 = img1.cuda(args.gpu, non_blocking=True)

                with autocast(enabled=args.amp):
                    target_logits_mask = model(img1)

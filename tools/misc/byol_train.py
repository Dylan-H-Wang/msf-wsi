"""  
copy from moco_train
ablation on ssl strategy
"""
...
def main_worker(gpu, ngpus_per_node, arg):
    ...
    # )  # TODO: hardcoded
    context_branch = BYOL(models.__dict__[args.arch], image_size = 256, hidden_layer = 'avgpool')
    target_branch = BYOL(models.__dict__[args.arch], image_size = 256, hidden_layer = 'avgpool')
    ...
    if args.distributed:
        # Apply SyncBN
        context_branch = torch.nn.SyncBatchNorm.convert_sync_batchnorm(context_branch)
        target_branch = torch.nn.SyncBatchNorm.convert_sync_batchnorm(target_branch)
        ...
        if args.gpu is not None:
            ...
            context_branch.cuda(args.gpu)
            target_branch.cuda(args.gpu)
            # When using a single GPU per process and per
            # DistributedDataParallel, we need to divide the batch size
            # ourselves based on the total number of GPUs we have
            args.batch_size = int(args.batch_size / ngpus_per_node)
            args.workers = int((args.workers + ngpus_per_node - 1) / ngpus_per_node)
            context_branch = torch.nn.parallel.DistributedDataParallel(
                context_branch, device_ids=[args.gpu], find_unused_parameters=True
            )
            target_branch = torch.nn.parallel.DistributedDataParallel(
                target_branch, device_ids=[args.gpu], find_unused_parameters=True
            )
        else:
            model.cuda()
    ...
    optim_params1 = context_branch.parameters()
    optim_params2 = target_branch.parameters()

    optim_params = [
        {"params": optim_params1},
        {"params": optim_params2},  # TODO: hardcoded
    ]
    ...
    # Data loading code
    context_aug = [
        albu.RandomResizedCrop(224, 224, scale=(0.6, 1.0), p=1),
                ...
                save_checkpoint(
                    {
                        "epoch": epoch + 1,
                        "arch": args.arch,
                        "context_state_dict": context_branch.state_dict(),
                        "target_state_dict": target_branch.state_dict(),
                    },
                    is_best=False,
...
def train(train_loader, context_branch, target_branch, optimizer, epoch, scaler):
    ...
    for it, (context_img, target_img, jigsaw_idx) in enumerate(train_loader):
        ...
        with autocast(enabled=args.amp):
            context_loss = context_branch(context_img[0], context_img[1])
            target_loss = target_branch(target_img[0], target_img[1])

            loss = context_loss + target_loss

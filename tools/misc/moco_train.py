...
def main_worker(gpu, ngpus_per_node, arg):
    ...
    # )  # TODO: hardcoded
    context_branch = MoCo(models.__dict__[args.arch])
    target_branch = MoCo(models.__dict__[args.arch], K=512*16)
    ...
    if args.distributed:
        ...
        if args.gpu is not None:
            ...
            context_branch = torch.nn.parallel.DistributedDataParallel(
                context_branch, device_ids=[args.gpu]
            )
            target_branch = torch.nn.parallel.DistributedDataParallel(
                target_branch, device_ids=[args.gpu]
            )
...
def train(train_loader, context_branch, target_branch, optimizer, epoch, scaler):
    ...
    for it, (context_img, target_img, jigsaw_idx) in enumerate(train_loader):
        ...
        with autocast(enabled=args.amp):
            context_logits, context_labels = context_branch(context_img[0], context_img[1])
            target_logits, target_labels = target_branch(target_img[0], target_img[1])

            context_loss = criterion(context_logits, context_labels)

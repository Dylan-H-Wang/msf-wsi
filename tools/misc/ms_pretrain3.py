"""
Add deepcluster to the model compared with ms_pretrain.py
"""
import argparse
import os
import sys
...
def main_worker(gpu, ngpus_per_node, arg):
    ...
    model = MSClusterModel(
        models.__dict__[args.arch], args.scale, args.nmb_prototypes, args.dim, args.pred_dim
    )  # TODO: hardcoded
    ...
    if args.data_name == "hubmap":
        ...
        train_dataset = BcssPretrainDatasetMS(
            args.data,
            (
                albu.Compose(context_aug),
                albu.Compose(target_aug),
    ...
    mb_path = os.path.join(args.log_dir, "mb" + str(args.rank) + ".pth")
    ...
    for epoch in range(args.epochs):
        ...
        # train for one epoch
        loss, local_memory_index, local_memory_embeddings = train(
            train_loader,
            model,
            optimizer,
            epoch,
...
def train(train_loader, model, optimizer, epoch, scaler, local_memory_index, local_memory_embeddings):
    ...
    cluster_loss = nn.CrossEntropyLoss(ignore_index=-100).cuda(args.gpu)
    if args.use_clr:
        cls_loss = nn.CrossEntropyLoss().cuda(args.gpu)

    assignments = cluster_memory(model, local_memory_index, local_memory_embeddings, len(train_loader.dataset))
    logger.info('Clustering for epoch {} done.'.format(epoch))

    end = time.time()
    start_idx = 0
    for it, (idx, context_img, target_img, jigsaw_idx) in enumerate(train_loader):
        ...
        with autocast(enabled=args.amp):
            ...
            # Cluster loss
            scores = outputs[4] / 0.1 # TODO: hardcoded
            targets = assignments[0][idx].repeat(2).cuda(non_blocking=True)
            cluster_clr_loss = cluster_loss(scores, targets)

            loss = context_loss + target_loss + fuser_loss + cluster_clr_loss

            if args.use_clr:
                fuser_cls_loss = cls_loss(
                    outputs[3],
                    torch.cat([jigsaw_idx[0].flatten(), jigsaw_idx[1].flatten()], 0),
                )
                loss = loss + fuser_cls_loss
        ...
        for i, crop_idx in enumerate(args.crops_for_assign):
            local_memory_embeddings[i][start_idx : start_idx + bs] = \
                outputs[3][crop_idx * bs : (crop_idx + 1) * bs]
        start_idx += bs
    ...
    return losses.avg, local_memory_index, local_memory_embeddings
...
def init_memory(dataloader, model):
    ...
    with torch.no_grad():
        ...
        for index, context_img, target_img, jigsaw_idx in dataloader:
            nmb_unique_idx = context_img[0].size(0)
            ...
            outputs = torch.tensor_split(outputs, 2)
...
def cluster_memory(model, local_memory_index, local_memory_embeddings, size_dataset, nmb_kmeans_iters=10):
    ...
    with torch.no_grad():
        for i_K, K in enumerate(args.nmb_prototypes):
            ...
            # getattr(model.module.cluster_prototype, "prototypes" + str(i_K)).weight.copy_(centroids)
            model.module.cluster_prototype.weight.copy_(centroids)

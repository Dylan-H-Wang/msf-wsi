...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.weights is not None:
        if os.path.isfile(args.weights):
            logger.info(
                f"=> loading slf_hist pretrained weights {args.weights} into encoder"
            )
            ...
            logger.info(
                f"=> loaded slf_hist pretrained weights {args.weights} into encoder"
            )
    ...
    train_dataset = BcssSegDataset(args.train_data, train_aug, frac=args.frac, fold=args.fold)
    val_dataset = BcssSegDatasetVal(args.train_data, val_aug, fold=args.fold)

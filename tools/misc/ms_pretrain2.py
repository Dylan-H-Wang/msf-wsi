"""  
In this training strategy, model does not contrast context and target branch,
instead, it contrast only their concatenations.

Data augmentation is also changed.
"""
...
def main_worker(gpu, ngpus_per_node, arg):
    ...
    target_aug = [
        # albu.RandomResizedCrop(1024, 1024, scale=(0.6, 1.0), p=1),
        albu.ColorJitter(0.4, 0.4, 0.4, 0.1, p=0.8),
        albu.ToGray(p=0.2),
        albu.OneOf(
            [albu.GaussianBlur(sigma_limit=[0.1, 2.0], p=0.5), albu.Sharpen(p=0.5)],

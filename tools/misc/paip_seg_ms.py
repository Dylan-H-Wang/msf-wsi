...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.use_ms:
        model = MSUnet(encoder_name=args.arch, encoder_weights=encoder_weights, classes=4)
    else:
        model = smp.Unet(
    ...
    # TODO: experiment MS imagenet-init model

    if args.weights is not None:
        if os.path.isfile(args.weights):
            logger.info(
                f"=> loading SLF-WSI pretrained weights {args.weights} into encoder"
            )
            ...
            if args.use_ms:
                ...
                for k in list(state_dict.keys()):
                    # retain only encoder_q up to before the embedding layer
                    if k.startswith("module.target_encoder") and not k.startswith(
                        "module.target_encoder.fc"
                    ):
                        # remove prefix
                        state_dict[k[len("module.target_encoder.") :]] = state_dict[k]
            ...
            logger.info(
                f"=> loaded SLF-WSI pretrained weights {args.weights} into encoder"
            )
    ...
    elif encoder_weights:
        logger.info(f"=> Model is ImageNet initialised!")

    else:
        logger.warning(f"=> Model is RANDOMLY initialised!")
    ...
    criterion = smp.losses.DiceLoss(
        smp.losses.MULTICLASS_MODE, classes=[1, 2, 3], from_logits=True
    )
    ...
    val_dataset = PaipSegDatasetValMS(args.val_data, val_aug, fold=args.fold)
    ...
    tissue_f1_recorder = BestRecorder("max")
    whole_f1_recorder = BestRecorder("max")
    viable_f1_recorder = BestRecorder("max")
    ...
    for epoch in range(args.epochs):
        ...
        if not args.multiprocessing_distributed or (
            ...
            tissue_f1, whole_f1, viable_f1 = scores.mean(0)

            best_f1_micro, is_best = micro_f1_recorder.update(val_f1_micro)
            best_f1_macro, _ = macro_f1_recorder.update(val_f1_macro)
            best_tissue_f1, _ = tissue_f1_recorder.update(tissue_f1_)
            best_whole_f1, _ = whole_f1_recorder.update(whole_f1_)
            best_viable_f1, _ = viable_f1_recorder.update(viable_f1_)
            ...
            logger.info(
                ...
                f"TISSUE F1: {tissue_f1:.4f}/{tissue_f1_:.4f}/{best_tissue_f1:.4f}, WHOLE F1: {whole_f1:.4f}/{whole_f1_:.4f}/{best_whole_f1:.4f}\n"
                f"VIABLE F1: {viable_f1:.4f}/{viable_f1_:.4f}/{best_viable_f1:.4f}\n"
...
def validate(loader, model, epoch, args):
    ...
    with torch.no_grad():
        ...
        for i, (images, masks) in enumerate(loader):
            ...
            tissue_f1_ = smp.metrics.f1_score(
                    tp, fp, fn, tn, reduction="weighted", class_weights=[1, 0, 0]
            ) * 3
            whole_f1_ = smp.metrics.f1_score(
                    tp, fp, fn, tn, reduction="weighted", class_weights=[0, 1, 0]
            ) * 3
            viable_f1_ = smp.metrics.f1_score(
                    tp, fp, fn, tn, reduction="weighted", class_weights=[0, 0, 1]
            ) * 3
            ...
            tissue_f1.append(tissue_f1_.item())
            whole_f1.append(whole_f1_.item())
            viable_f1.append(viable_f1_.item())
...
def validate2(loader, model, epoch, args):
    batch_time = AverageMeter("Time", ":6.3f")
    data_time = AverageMeter("Data", ":6.3f")
    progress = ProgressMeter(
        len(loader),
    ...
    with torch.no_grad():
        ...
        for i, (context_img, target_img, target_masks) in enumerate(loader):
            # measure data loading time
            data_time.update(time.time() - end)

            if args.gpu is not None:
                context_img = context_img.cuda(args.gpu, non_blocking=True)
                target_img = target_img.cuda(args.gpu, non_blocking=True)
                target_masks = target_masks.cuda(args.gpu, non_blocking=True)

            # compute output and loss
            with autocast(enabled=args.amp):
                _, target_logits_mask = model(context_img, target_img)

            pred_mask = torch.argmax(target_logits_mask, dim=1)
            tp, fp, fn, tn = smp.metrics.get_stats(
                pred_mask.long() - 1,
                target_masks.long() - 1,
                mode="multiclass",
        ...
        fns = torch.cat(fns, dim=0)
        tns = torch.cat(tns, dim=0)

        scores = smp.metrics.f1_score(tps, fps, fns, tns, reduction=None)
        val_f1_micro = smp.metrics.f1_score(
            tps, fps, fns, tns, reduction="micro-imagewise"
        )
        val_f1_macro = smp.metrics.f1_score(
            tps, fps, fns, tns, reduction="macro-imagewise"
        )
        val_acc_micro = smp.metrics.accuracy(
            tps, fps, fns, tns, reduction="micro-imagewise"
        )
        val_acc_macro = smp.metrics.accuracy(
            tps, fps, fns, tns, reduction="macro-imagewise"
        )
        tissue_f1_, whole_f1_, viable_f1_ = scores.mean(0)

        # val_f1_micro = smp.metrics.f1_score(tp, fp, fn, tn, reduction="micro", class_weights=[0, 2, 6])
        # val_f1_macro = smp.metrics.f1_score(tp, fp, fn, tn, reduction="macro", class_weights=[0, 2, 6]) * 3 / 2
...
def test(loader, model, n_val, epoch, args):
    batch_time = AverageMeter("Time", ":6.3f")
    data_time = AverageMeter("Data", ":6.3f")
    progress = ProgressMeter(
        len(loader),
        [batch_time, data_time],
        prefix=f"Val epoch: [{n_val}/{epoch}]",
    )
    logger = logging.getLogger("SLF-WSI")
    ...
    with torch.no_grad():
        ...
        for i, (context_img, target_img, tissue_mask, is_empty) in enumerate(loader):
            # measure data loading time
            data_time.update(time.time() - end)
            empty_idx = torch.asarray(is_empty)

            if args.use_ms:
                if args.gpu is not None:
                    context_img_ = context_img[~empty_idx].cuda(args.gpu, non_blocking=True)
                    target_img_ = target_img[~empty_idx].cuda(args.gpu, non_blocking=True)

                # compute output and loss
                with autocast(enabled=args.amp):
                    _, target_logits_mask = model(context_img_, target_img_)
                    preds.append(target_logits_mask.detach().cpu())
...
if __name__ == "__main__":
    ...
    parser.add_argument("--val-data", type=str, help="test set path")

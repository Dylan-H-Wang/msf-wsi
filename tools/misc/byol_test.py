...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.use_ms:
        model = MSUnet(encoder_name=args.arch, encoder_weights=encoder_weights, classes=6)
    else:
        model = smp.Unet(
    ...
    if args.weights is not None:
        if os.path.isfile(args.weights):
            ...
            state_dict1 = checkpoint["context_state_dict"]
            state_dict2 = checkpoint["target_state_dict"]
            ...
            for k in list(state_dict1.keys()):
                ...
                if k.startswith("module.online_encoder.net") and not k.startswith(
                    "module.online_encoder.net.fc"
                ):
                    # remove prefix
                    context_state_dict[
                        k[len("module.online_encoder.net.") :]
                    ] = state_dict1[k]
    ...
    with torch.no_grad():
        ...
        for i, (images, masks) in enumerate(loader):
            ...
            if args.use_ms:
                ...
                images = images[0]
                masks = masks[0]
                images_split = torch.split(images, 64)
                masks_split = torch.split(masks, 64)

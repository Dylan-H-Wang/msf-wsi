...
pre-defined ratio of target patches.
(same as ms_pretrain5_2.py)

add IIC for concatanation
"""
...
def train(train_loader, model, optimizer, epoch, scaler):
    ...
    for it, (context_img, target_img, jigsaw_idx) in enumerate(train_loader):
        ...
        with autocast(enabled=args.amp):
            ...
            cluster_loss = IIC_loss(outputs[3][0], outputs[3][1]) + IIC_loss(outputs[3][2], outputs[3][3])
            loss = context_loss + target_loss + fuser_loss + cluster_loss

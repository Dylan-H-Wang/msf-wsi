...
def main_worker(gpu, ngpus_per_node, args):
    ...
    from msynet.models.YclassRes18Net import YclassRes18Net
    ...
    model = YclassRes18Net(cfg=cfg).to(device='cuda')
...
def train(loader, model, criterion, optimizer, epoch, scaler, args):
    ...
    for i, (images, masks) in enumerate(loader):
        ...
        with autocast(enabled=args.amp):
            segmentation_logits = model(images[1], images[0])
            loss = criterion(segmentation_logits, masks[1])

        losses.update(loss.item(), args.batch_size)

        pred_mask = torch.argmax(segmentation_logits.detach(), dim=1)
        ...
        del loss, images, masks, segmentation_logits, pred_mask
...
def validate(loader, model, epoch, args):
    ...
    with torch.no_grad():
        ...
        for i, (images, masks) in enumerate(loader):
            ...
            context_imgs_split = torch.split(context_imgs, 64)
            target_imgs_split = torch.split(target_imgs, 64)
            ...
            for img1, img2 in zip(
                ...
                # compute output and loss
                with autocast(enabled=args.amp):
                    segmentation_logits = model(img2, img1)
                    preds.append(segmentation_logits.detach().cpu())

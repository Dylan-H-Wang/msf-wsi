"""  
modified from ms_pretrain_ablation

Ablation studies for backbone model choice
"""
...
from utils.utils import increment_path, Normalize
...
def main_worker(gpu, ngpus_per_node, arg):
    ...
    if args.arch == "resnet18":
        model = ABModel(resnet2.__dict__[args.arch], args.scale, args.mask_ratio/100)
    
    elif args.arch == "resnet34":
        model = ABModel(resnet2.__dict__[args.arch], args.scale, args.mask_ratio/100)

    elif args.arch == "densenet":
        model = ABModelDenseNet(get_encoder, args.scale, args.mask_ratio/100)

    elif args.arch == "efficientnet":
        model = ABModelEfficient(get_encoder, args.scale, args.mask_ratio/100)

    elif args.arch == "regnet":
        model = ABModelRegNet(get_encoder, args.scale, args.mask_ratio/100)

    elif args.arch == "mit":
        model = ABModelMiT(get_encoder, args.scale, args.mask_ratio/100)
    ...
    if args.distributed:
        ...
        if args.gpu is not None:
            ...
            model = torch.nn.parallel.DistributedDataParallel(
                model, device_ids=[args.gpu], find_unused_parameters=(args.arch == "efficientnet")
            )
        else:
            model.cuda()
    ...
    target_params = [
        i[1]
        for i in filter(
            lambda kv: kv[0].startswith("target_"), model.module.named_parameters(),
        )
    ]
    inter_params = [
        i[1]
        for i in filter(
            lambda kv: kv[0].startswith("inter_"), model.module.named_parameters(),
        )
    ]

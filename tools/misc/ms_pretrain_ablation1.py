"""  
Same as ms_pretrain7

Ablation studies for CTFM and DSL 
"""
...
def main_worker(gpu, ngpus_per_node, arg):
    ...
    if args.mode == 1:
        model = MSModelAB1_1(
            resnet2.__dict__[args.arch], Normalize(args.mean, args.std), args.scale
        )  # TODO: hardcoded
    elif args.mode == 2:
        model = MSModelAB1_2(
            resnet2.__dict__[args.arch], Normalize(args.mean, args.std), args.scale
    ...
    if args.data_name == "hubmap":
        ...
        bcss_dst = None
        if args.mode == 1:
            bcss_dst = BcssPretrainDatasetMSAB1_1
        elif args.mode == 2:
            bcss_dst = BcssPretrainDatasetMSAB1_2
        elif args.mode == 3:
            bcss_dst = BcssPretrainDatasetMSAB1_3

        train_dataset = bcss_dst(
            args.data,
            (
                albu.Compose(context_aug),
                albu.Compose(target_aug),

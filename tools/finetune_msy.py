...
from src.models.msynet.models.YclassRes18Net import YclassRes18Net
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # create model
    logger.info("=> creating model msY-Net")
    ...
    if args.weights and os.path.isfile(args.weights):
        ...
        weights = {"context": context_state_dict, "target": target_state_dict}
    ...
    cfg = {
        'num_classes': len(args.class_names)+1,
        'num_channels': 3,
    ...
    model = YclassRes18Net(cfg=cfg, weights=weights).to(device='cuda')
...
def train(loader, model, criterion, optimizer, epoch, scaler, args):
    ...
    for i, (images, masks) in enumerate(loader):
        ...
        with autocast(enabled=args.amp):
            target_logits_mask = model(images[1], images[0])
...
def validate(loader, model, epoch, args):
    ...
    for i, (images, masks) in enumerate(loader):
        ...
        context_imgs_split = torch.split(context_imgs, args.batch_size*2)
        target_imgs_split = torch.split(target_imgs, args.batch_size*2)
        ...
        for img1, img2 in zip(context_imgs_split, target_imgs_split):
            ...
            # compute output and loss
            with autocast(enabled=args.amp):
                target_logits_mask = model(img2, img1)

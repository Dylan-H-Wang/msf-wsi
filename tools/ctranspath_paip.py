...
from src.models.ctranspath import SwinUnet
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # create model
    logger.info("=> creating CTransPath")
    model = SwinUnet(num_classes=4)
    # print(model)
 
    if os.path.isfile(args.weights):
        logger.info(f"=> loading CTransPath pretrained weights {args.weights}")
        state_dict = torch.load(args.weights, map_location="cpu")["model"]

        patch_embed_state_dict = OrderedDict()
        ...
        for k in list(state_dict.keys()):
            ...
            if k.startswith("patch_embed"):
                # remove prefix
                patch_embed_state_dict[k[len("patch_embed.") :]] = state_dict[k]

            elif k.startswith("layers"):
                layer_state_dict[k[len("layers.") :]] = state_dict[k]

            # delete renamed or unused k
            del state_dict[k]

        model.patch_embed.load_state_dict(patch_embed_state_dict)
        model.layers.load_state_dict(layer_state_dict)

        logger.info(f"=> loaded CTransPath pretrained weights {args.weights} into encoder")
    else:
...
def train(loader, model, criterion, optimizer, epoch, scaler, args):
    ...
    for i, (images, masks) in enumerate(loader):
        ...
        with autocast(enabled=args.amp):
            target_logits_mask = model(images[1])
            loss = criterion(target_logits_mask, masks[1])
        ...
        del loss, images, masks, target_logits_mask, pred_mask
...
def validate(loader, model, epoch, args):
    ...
    for i, (images, masks) in enumerate(loader):
        ...
        target_imgs_split = torch.split(target_imgs, args.batch_size)

        preds = []
        for img in target_imgs_split:
            img = img.cuda(args.gpu, non_blocking=True)

            # compute output and loss
            with autocast(enabled=args.amp):
                target_logits_mask = model(img)

...
class BcssSegDataset(Dataset):
    def __init__(self, data_path, transforms, mask_on=True, frac=1, threshold=0.1, fold=0) -> None:
        super().__init__()
    ...
    def __getitem__(self, index: int) -> Tensor:
        ...
        mask = np.asarray(Image.open(mask_path)) if self.mask_on else -1
    ...
    def _prepare(self) -> None:
        logger = logging.getLogger("SLF-WSI."+__name__)
        ...
        # self.data_df = data_df.head(int(len(data_df)*self.frac)).reset_index(drop=True)
...
class BcssSegDatasetVal(Dataset):
...
class BcssSegDatasetMS(Dataset):
    ...
    def __getitem__(self, index: int) -> Tensor:
        ...
        if self.transforms is not None:
            ...
            # self.transforms[2].transforms[0] = albu.Resize(286, 286, always_apply=True)
...
class BcssSegDatasetValMS(Dataset):
    def __init__(self, data_path, transforms, threshold=0.1, fold=0) -> None:
        super().__init__()
...
class BcssPretrainDataset(Dataset):
    def __init__(self, data_path, transforms, frac=1, return_index=False, threshold=0.1, fold=0) -> None:
        super().__init__()

        self.data_path = data_path
        self.csv_path = data_path + f"/data.csv"
        self.transforms = transforms
        self.frac = frac

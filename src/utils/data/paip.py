...
class PaipSegDatasetVal(Dataset):
    ...
    def __getitem__(self, index: int) -> Tensor:
        ...
        return target_imgs, target_masks
...
# TODO
class PaipSegDatasetTestMS(Dataset):
    def __init__(self, data_path, filename, transforms, threshold=0.1) -> None:
        super().__init__()
    ...
    def __getitem__(self, index: int) -> Tensor:
        is_empty = "empty" in self.context_imgs[index]

        if is_empty:
            context_img = torch.zeros((3, 256, 256), dtype=torch.float32)
            target_img = torch.zeros((3, 256, 256), dtype=torch.float32)
            tissue_mask = torch.array(
                Image.open(os.path.join(self.data_path, self.tissue_masks[index])),
                dtype=torch.float32,
            )

        else:
            context_img = np.array(
                Image.open(os.path.join(self.data_path, self.context_imgs[index]))
            )
            target_img = np.array(Image.open(os.path.join(self.data_path, self.target_imgs[index])))
            tissue_mask = torch.array(
                Image.open(os.path.join(self.data_path, self.tissue_masks[index])),
                dtype=torch.float32,
            )
            ...
            if self.transforms is not None:
                context_img = self.transforms(image=context_img)["image"]
                target_img = self.transforms(image=target_img)["image"]

        return context_img, target_img, tissue_mask, is_empty

    def _prepare(self) -> None:
        logger = logging.getLogger("DSF-WSI." + __name__)
        data_df = pd.read_csv(self.csv_path)
        self.data_df = data_df[data_df["filename"] == self.filename].reset_index(drop=True)
        logger.info(f"Reading {len(self.data_df)} files in {self.csv_path}...")

        self.context_imgs = self.data_df["context_img"].tolist()
        self.target_imgs = self.data_df["target_img"].tolist()
        self.tissue_masks = self.data_df["tissue_mask"].tolist()
        assert len(self.context_imgs) == len(self.target_imgs) == len(self.tissue_masks)

...
from segmentation_models_pytorch.decoders.unet.decoder import DecoderBlock, CenterBlock, UnetDecoder
...
class ContextUnetDecoder(UnetDecoder):
    def forward(self, *features):
        ...
        for i, decoder_block in enumerate(self.blocks):
            ...
            if i==1:
                context_feats = x[:,:,16-4:16+4,16-4:16+4] #TODO: hardcoded for hooknet
...
class TargetUnetDecoder(nn.Module):
    ...
    def __init__(
        ...
        in_channels = [head_channels+128] + list(decoder_channels[:-1]) #TODO: hardcoded
...
class HookNet(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.context_branch = ContextUnet(encoder_name, encoder_depth, encoder_weights, decoder_use_batchnorm, decoder_channels, decoder_attention_type, in_channels, classes, activation, aux_params)
        self.target_branch = TargetUnet(encoder_name, encoder_depth, encoder_weights, decoder_use_batchnorm, decoder_channels, decoder_attention_type, in_channels, classes, activation, aux_params)

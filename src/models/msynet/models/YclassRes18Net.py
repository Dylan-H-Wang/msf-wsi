...
from ..base import BaseNet, ResBottleneckBlock, ResEncoderBlock, SkipBlock, DecoderBlock
from ..core import MultiScaleMergeBlock
...
class YclassRes18Net(BaseNet):
    ...
    def __init__(self, cfg, weights=None):
        ...
        base_model_tail = models.resnet18()
        if weights: 
            msg1 = base_model.load_state_dict(weights["target"], strict=False)
            msg2 = base_model_tail.load_state_dict(weights["context"], strict=False)
            print(f"Missing weights: target - {msg1}, context - {msg2}")

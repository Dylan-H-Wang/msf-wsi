...
CUDA_VISIBLE_DEVICES=0 TORCH_DISTRIBUTED_DEBUG=DETAIL python ./tools/ssl_train_2.py \
...
log_path="./logs/paip/c16_train_set"
for i in {0049..0199..50}
do
    CUDA_VISIBLE_DEVICES=0 TORCH_DISTRIBUTED_DEBUG=DETAIL python ./tools/ssl_ft_paip.py  \
        ...
        --weights ./logs/camelyon/train_set/checkpoint_${i}.pth.tar \
        --amp --tensorboard --dist-url "tcp://127.0.0.1:50001"
done



LOG_PATH="./logs/camelyon/all_set"
CUDA_VISIBLE_DEVICES=0 TORCH_DISTRIBUTED_DEBUG=DETAIL python ./tools/ssl_train_2.py \
    -a resnet18 -j 4 -b 32 --epochs 300 --lr 1e-3 \
    --multiprocessing-distributed --world-size 1 --rank 0 \
    --data-name camelyon16_all --data ./data/Dataset001_Camelyon16-1024 \
    --mean 0.5783 0.3970 0.6128 \
    --std 0.2424 0.2379 0.1918 \
    --log-dir ${LOG_PATH} \
    --save-freq 50 \
...
log_path="./logs/paip/c16_all_set"
for i in {0049..0199..50}
do
    CUDA_VISIBLE_DEVICES=0 TORCH_DISTRIBUTED_DEBUG=DETAIL python ./tools/ssl_ft_paip.py  \
        -j 4 -b 64 --epochs 50 --lr 1e-3 \
        ...
        --std 0.14612035 0.1648203  0.12789637 \
        --log-dir ${log_path}/test_${i} \
        --weights ./logs/camelyon/all_set/checkpoint_${i}.pth.tar \
        --amp --tensorboard --dist-url "tcp://127.0.0.1:50001"
done
